  ...
  return (
    <Card className="overflow-hidden animate-fade-in border-black/5 mac-shadow">
      ...
      <Dialog open={!!screenshotOrder} onOpenChange={() => setScreenshotOrder(null)}>
        <DialogContent className="sm:max-w-[600px] rounded-xl border-slate-100 shadow-xl p-0 overflow-hidden">
          ...
          <div className="bg-slate-100/50 p-6 flex flex-col items-center justify-center min-h-[300px]">
            <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 text-center space-y-4 max-w-sm w-full">
              ...
              <p className="text-sm font-medium text-slate-500">₹{((screenshotOrder?.quantity || 1) * 79900).toLocaleString()}</p>
              <div className="pt-4 border-t border-slate-100 text-xs text-slate-400 font-mono text-left space-y-2">
                <div className="flex justify-between"><span className="uppercase font-bold">Transaction ID</span><span className="text-slate-700">TXN{screenshotOrder?.order_id.replace(/\D/g, '')}</span></div>
                <div className="flex justify-between"><span className="uppercase font-bold">Paid With</span><span className="text-slate-700">ICICI Bank •••• 1234</span></div>
              </div>
            </div>
            <p className="text-[10px] text-slate-400 mt-6 uppercase font-bold tracking-widest text-center">Mock Screenshot Render</p>
          </div>
        </DialogContent>
      </Dialog>
    </Card>
